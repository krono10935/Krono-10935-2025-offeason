...
public class VisionIOPhoton implements VisionIO {
     ...
     * @param referencePoseSupplier the refrence pose soplire
     */
    public VisionIOPhoton(CamerasConstants camerasConstants, AprilTagFieldLayout fieldLayout, Supplier<Pose2d> referencePoseSupplier){
    
        camera = new PhotonCamera(camerasConstants.CAMERA_NAME);
        poseEstimator = new PhotonPoseEstimator(fieldLayout, camerasConstants.MAIN_STRATEGY, camerasConstants.ROBOT_TO_CAMERA );
...
private VisionFrame generateFrame(PhotonPipelineResult result) {
    ...
    double averageTargetDistanceMeters = 0.0;
    ...
    if (result.multitagResult.isPresent()) {
        ...
        for (var target : result.targets) {
            double distanceToTag = target.getBestCameraToTarget()
                                        .getTranslation()
                                        .getNorm();
            averageTargetDistanceMeters += distanceToTag;
        }
    } 
    /**
     ...
     */
         ...
         */
        averageTargetDistanceMeters = result.getTargets().get(0)
    ...
    return new VisionFrame(
        ...
        pose.estimatedPose,
        poseAmbiguity,
        averageTargetDistanceMeters / result.getTargets().size(),
